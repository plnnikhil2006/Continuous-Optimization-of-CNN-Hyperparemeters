...
1. [Executive Summary and Problem Overview](#1-executive-summary-and-problem-overview)
2. [Theoretical Background and Motivation](#2-theoretical-background-and-motivation)
   - [2.1 The Curse of Dimensionality in Grid Search](#21-the-curse-of-dimensionality-in-grid-search)
   - [2.2 The Case for Metaheuristics](#22-the-case-for-metaheuristics)
3. [Mathematical Formulation](#3-mathematical-formulation)
   - [3.1 Continuous Decision Variables](#31-continuous-decision-variables)
   - [3.2 Objective Function Definition](#32-objective-function-definition)
   - [3.3 Boundary Constraints and Feasibility](#33-boundary-constraints-and-feasibility)
4. [Environment and CNN Architecture](#4-environment-and-cnn-architecture)
   - [4.1 Dataset Stratification and Subsampling](#41-dataset-stratification-and-subsampling)
   - [4.2 Layer-by-Layer Network Topology](#42-layer-by-layer-network-topology)
   - [4.3 Hardware and Computational Setup](#43-hardware-and-computational-setup)
5. [Algorithmic Deep Dive: Metaheuristics](#5-algorithmic-deep-dive-metaheuristics)
   - [5.1 Particle Swarm Optimization (PSO) Mathematics](#51-particle-swarm-optimization-pso-mathematics)
   - [5.2 Genetic Algorithm (GA) Evolutionary Operators](#52-genetic-algorithm-ga-evolutionary-operators)
6. [Project Architecture and Code Structure](#6-project-architecture-and-code-structure)
   - [6.1 Directory Tree](#61-directory-tree)
   - [6.2 Core Functional Modules](#62-core-functional-modules-inside-srcmainpy)
7. [Installation and Environment Setup](#7-installation-and-environment-setup)
8. [Usage and Execution Instructions](#8-usage-and-execution-instructions)
9. [Empirical Results and Performance](#9-empirical-results-and-performance)
   - [9.1 Quantitative Comparison Table](#91-quantitative-comparison-table)
   - [9.2 Convergence Trajectory Analysis](#92-convergence-trajectory-analysis)
10. [Deep Discussion and Landscape Analysis](#10-deep-discussion-and-landscape-analysis)
    - [10.1 Parameter Sensitivity (Anisotropy)](#101-parameter-sensitivity-anisotropy)
    - [10.2 Navigating Local Optima Traps](#102-navigating-local-optima-traps)
    - [10.3 Constraint Enforcement Strategies](#103-constraint-enforcement-strategies)
11. [Known Limitations and Future Scope](#11-known-limitations-and-future-scope)
12. [License and Academic Declaration](#12-license-and-academic-declaration)
...
PSO is a swarm-intelligence metaheuristic inspired by the social foraging behavior of bird flocks. It maintains a swarm of particles, where each particle i has a position Xi and a velocity Vi.
...
#### 1. Arithmetic Crossover (Rate = 0.80)

Standard binary point-crossover is mathematically invalid for continuous real numbers. Instead, children are generated via a linear interpolation (blend) of parent vectors:
...
|-- report/                         # Formal academic documentation.
...
2. Dataset Verification  
   The script checks the ./data directory and automatically downloads MNIST if missing.

3. PSO Phase  
   Initializes 10 particles and evaluates 100 CNN configurations.

Example log:

```text
PSO Iteration X/10 | Best Val Loss: 0.XXXX
```

4. GA Phase  
   Evolves 10 chromosomes through 10 generations.

Example log:

```text
GA Generation X/10 | Best Val Loss: 0.XXXX
```

5. Plot Generation  
   Convergence plots are displayed and automatically saved.
...
### Learning Rate Axis (Steep and Sensitive)

The objective landscape is incredibly steep along the alpha axis. Particles evaluating values outside the narrow 10^-3 to 10^-2 corridor experienced massive loss spikes. Any value approaching 10^-1 caused the Adam optimizer to overshoot minima entirely, failing to converge.

### Dropout Rate Axis (Flat and Robust)

Conversely, the landscape proved highly robust to changes in the dropout dimension. Configurations varying widely between 0.10 and 0.35 resulted in near-identical validation metrics. The algorithms correctly identified that precise structural regularization was less critical than achieving the optimal optimization step-size.
...
The Genetic Algorithm exhibited a clear tendency to plateau around Generation 5. Without a massive population (e.g., 100+) to sustain diverse genetic material, the tournament selection mechanism rapidly homogenized the gene pool. Once the population lost diversity, arithmetic crossover simply averaged similar numbers, trapping the algorithm in a shallow local optimum.

### PSO Momentum to the Rescue

PSO successfully escaped the early stagnation that trapped the GA. The inertia weight (w=0.5) provided mathematical momentum. Even when a particle reached a local minimum, its accumulated velocity forced it to overshoot the minimum slightly, allowing the swarm to traverse small loss ridges and settle into a deeper, superior basin of attraction by Iteration 8.
...
Constraints were enforced strictly via post-velocity positional clipping.

### In GA

Boundaries were enforced immediately following the Gaussian mutation step.

This guaranteed 100% functional feasibility and zero runtime crashes.
...
1. Subsampling Bias  
   Evaluations are performed on a 5,000-sample subset for only 1 to 2 epochs per trial.

2. Dimensional Scope Constraints  
   The search space is intentionally limited to two continuous variables.

3. Fixed Topology Architecture  
   The project optimizes operational hyperparameters but not architecture itself.

4. Multi-Objective Expansion  
   Future work could implement NSGA-II for simultaneous optimization of:
   - Validation loss
   - Parameter count
   - Memory footprint
...
This project, repository, and all associated code artifacts were developed as a formal submission for the Optimization Techniques course assignment.

All continuous metaheuristic implementations (Particle Swarm Optimization and Genetic Algorithm) were written entirely from scratch specifically for this hyperparameter tuning problem, relying only on standard NumPy vectorization logic.
...
This codebase is released under the MIT License for educational, academic, and non-commercial open-source purposes.
